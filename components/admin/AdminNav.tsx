  ...
  return (
    <nav className="bg-warm-900 text-warm-100">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 flex items-center justify-between h-14">
        {/* Left: tenant name + desktop links */}
        <div className="flex items-center gap-6 min-w-0">
          {/* Tenant switcher */}
          <div className="relative min-w-0">
            <button
              onClick={() => otherTenants.length > 0 && setSwitcherOpen(!switcherOpen)}
              className={`flex items-center gap-1.5 font-heading text-lg font-semibold text-white truncate ${
                otherTenants.length > 0 ? "cursor-pointer hover:text-amber-accent" : ""
              }`}
            >
              <span className="truncate">{currentTenantName || "MaRésa"}</span>
        ...
        {/* Right: desktop user info + mobile burger */}
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="lg:hidden w-10 h-10 flex items-center justify-center text-warm-300 hover:text-white transition-colors"
          >
            <svg
              width="22"
              height="22"
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden border-t border-white/10 animate-slide-down">
          ...
          <div className="border-t border-white/10 px-4 py-3 flex items-center justify-between">
            <span className="text-xs text-warm-400 truncate">{userEmail}</span>
            <button
              onClick={handleSignOut}
              className="text-xs text-warm-300 hover:text-white transition-colors flex-shrink-0 ml-3"
            >
              Déconnexion
            </button>
          </div>
